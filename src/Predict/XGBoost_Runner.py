...
# Paths to models
xgb_ml_path = r"C:\nba\NBA-Machine-Learning-Sports-Betting\Models\XGBoost_67.5%_ML-4.json"
xgb_uo_path = r"C:\nba\NBA-Machine-Learning-Sports-Betting\Models\XGBoost_53.6%_UO-9.json"


# Load XGBoost models
if not os.path.exists(xgb_ml_path) or not os.path.exists(xgb_uo_path):
    raise FileNotFoundError(f"XGBoost model files not found at {xgb_ml_path} or {xgb_uo_path}. Ensure they exist.")

xgb_ml = xgb.Booster()
xgb_ml.load_model(xgb_ml_path)
xgb_uo = xgb.Booster()
xgb_uo.load_model(xgb_uo_path)
...
def xgb_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, kelly_criterion):
    ...
    data = frame_uo.values.astype(float)
        ...
        if winner == 1:
            ...
            if under_over == 0:
                ...
                print(Fore.GREEN + home_team + Style.RESET_ALL + Fore.CYAN + f" ({winner_confidence}%)" + Style.RESET_ALL +
                      ...
                      Fore.MAGENTA + 'UNDER ' + Style.RESET_ALL + str(todays_games_uo[count]) + Style.RESET_ALL +
                      Fore.CYAN + f" ({un_confidence}%)" + Style.RESET_ALL)
            else:
                un_confidence = round(ou_predictions_array[count][0][1] * 100, 1)
                print(Fore.GREEN + home_team + Style.RESET_ALL + Fore.CYAN + f" ({winner_confidence}%)" + Style.RESET_ALL +
                      ' vs ' + Fore.RED + away_team + Style.RESET_ALL + ': ' +
                      Fore.BLUE + 'OVER ' + Style.RESET_ALL + str(todays_games_uo[count]) + Style.RESET_ALL +
                      Fore.CYAN + f" ({un_confidence}%)" + Style.RESET_ALL)
        else:
